...
/* ═══════════════════════════════════════════
   ...
   ═══════════════════════════════════════════ */
function TaskProgressSection({ taskId, workspaceId }: { taskId: string; workspaceId: string }) {
        ...
        <div style={{ marginTop: 20, paddingTop: 16, borderTop: '1px solid var(--border)' }}>
          {!wsShowDelete ? (
            <button
              className="btn btn-ghost btn-sm"
              style={{ color: 'var(--red-500)' }}
              onClick={() => setWsShowDelete(true)}
            >删除项目</button>
          ) : (
            <div style={{ background: 'var(--red-50)', border: '1px solid var(--red-100)', borderRadius: 'var(--radius)', padding: '12px 14px', fontSize: '0.78rem', color: 'var(--red-600)' }}>
              <div style={{ marginBottom: 8 }}>确定删除项目「{current?.name}」？此操作会删除所有任务和数据，不可撤销。</div>
              <div style={{ display: 'flex', gap: 6 }}>
                <button className="btn btn-ghost btn-xs" onClick={() => setWsShowDelete(false)}>取消</button>
                <button
                  className="btn btn-xs"
                  style={{ background: 'var(--red-500)', color: '#fff', border: 'none' }}
                  onClick={handleWsDelete}
                >
                  {wsEditSubmitting ? '删除中...' : '确认删除'}
                </button>
              </div>
            </div>
          )}
