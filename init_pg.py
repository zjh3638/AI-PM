...
ALL_TABLES = [
    ...
    """),
    ("ix_workflow_states_template_id", """
        CREATE INDEX IF NOT EXISTS ix_workflow_states_template_id ON workflow_states (template_id)
    """),
    ...
    """),
    ("ix_workflow_transitions_template_id", """
        CREATE INDEX IF NOT EXISTS ix_workflow_transitions_template_id ON workflow_transitions (template_id)
    """),
    ("ix_workflow_transitions_from_state_id", """
        CREATE INDEX IF NOT EXISTS ix_workflow_transitions_from_state_id ON workflow_transitions (from_state_id)
    """),
    ("ix_workflow_transitions_to_state_id", """
        CREATE INDEX IF NOT EXISTS ix_workflow_transitions_to_state_id ON workflow_transitions (to_state_id)
    """),
    ...
    """),
    ("ix_tasks_phase", """
        CREATE INDEX IF NOT EXISTS ix_tasks_phase ON tasks (phase)
    """),
    ("ix_tasks_task_type", """
    ...
    """),
    ("ix_tasks_iteration_id", """
        CREATE INDEX IF NOT EXISTS ix_tasks_iteration_id ON tasks (iteration_id)
    """),
    ("ix_tasks_parent_id", """
    ...
    """),
    ("ix_requirement_inbox_status", """
        CREATE INDEX IF NOT EXISTS ix_requirement_inbox_status ON requirement_inbox (status)
    """),
    ("ix_requirement_inbox_submitter_id", """
        CREATE INDEX IF NOT EXISTS ix_requirement_inbox_submitter_id ON requirement_inbox (submitter_id)
    """),
...
def _make_seed_statements(admin_password: str = "admin123"):
    ...
    return [
        ...
        # ── 完整研发流程的状态和迁移 ──
        ("INSERT INTO workflow_states (id, template_id, name, \"order\", category) VALUES "
         "('fs-todo',        'tmpl-full', '待办',    1, 'BACKLOG')  ON CONFLICT (id) DO NOTHING"),
        ("INSERT INTO workflow_states (id, template_id, name, \"order\", category) VALUES "
         "('fs-in-progress', 'tmpl-full', '进行中',  2, 'ACTIVE')   ON CONFLICT (id) DO NOTHING"),
        ("INSERT INTO workflow_states (id, template_id, name, \"order\", category) VALUES "
         "('fs-review',     'tmpl-full', '审核中',  3, 'REVIEW')    ON CONFLICT (id) DO NOTHING"),
        ("INSERT INTO workflow_states (id, template_id, name, \"order\", category) VALUES "
         "('fs-done',       'tmpl-full', '已完成',  4, 'DONE')      ON CONFLICT (id) DO NOTHING"),
        ("INSERT INTO workflow_transitions (id, template_id, from_state_id, to_state_id, name) VALUES "
         "('ft-todo-pg',  'tmpl-full', 'fs-todo',        'fs-in-progress', '开始')  ON CONFLICT (id) DO NOTHING"),
        ("INSERT INTO workflow_transitions (id, template_id, from_state_id, to_state_id, name) VALUES "
         "('ft-pg-rev',   'tmpl-full', 'fs-in-progress', 'fs-review',     '提交审核') ON CONFLICT (id) DO NOTHING"),
        ("INSERT INTO workflow_transitions (id, template_id, from_state_id, to_state_id, name) VALUES "
         "('ft-rev-pg',   'tmpl-full', 'fs-review',      'fs-in-progress', '驳回')   ON CONFLICT (id) DO NOTHING"),
        ("INSERT INTO workflow_transitions (id, template_id, from_state_id, to_state_id, name) VALUES "
         "('ft-rev-done', 'tmpl-full', 'fs-review',      'fs-done',       '通过')   ON CONFLICT (id) DO NOTHING"),
        ("INSERT INTO workflow_transitions (id, template_id, from_state_id, to_state_id, name) VALUES "
         "('ft-done-pg',  'tmpl-full', 'fs-done',        'fs-in-progress', '重新打开') ON CONFLICT (id) DO NOTHING"),
        ("INSERT INTO workflow_transitions (id, template_id, from_state_id, to_state_id, name) VALUES "
         "('ft-todo-done','tmpl-full', 'fs-todo',        'fs-done',       '关闭')   ON CONFLICT (id) DO NOTHING"),

        # ── 轻量专题流程的状态和迁移 ──
        ('INSERT INTO workflow_states (id, template_id, name, "order", category) VALUES '
         "('ls-planning', 'tmpl-lite', '计划中', 1, 'BACKLOG') ON CONFLICT (id) DO NOTHING"),
        ('INSERT INTO workflow_states (id, template_id, name, "order", category) VALUES '
         "('ls-doing',    'tmpl-lite', '执行中', 2, 'ACTIVE')  ON CONFLICT (id) DO NOTHING"),
        ('INSERT INTO workflow_states (id, template_id, name, "order", category) VALUES '
         "('ls-review',   'tmpl-lite', '审核中', 3, 'REVIEW')  ON CONFLICT (id) DO NOTHING"),
        ('INSERT INTO workflow_states (id, template_id, name, "order", category) VALUES '
         "('ls-done',     'tmpl-lite', '完成',   4, 'DONE')    ON CONFLICT (id) DO NOTHING"),
        ('INSERT INTO workflow_transitions (id, template_id, from_state_id, to_state_id, name) VALUES '
         "('lt-plan-do',   'tmpl-lite', 'ls-planning', 'ls-doing',   '启动')   ON CONFLICT (id) DO NOTHING"),
        ('INSERT INTO workflow_transitions (id, template_id, from_state_id, to_state_id, name) VALUES '
         "('lt-do-rev',    'tmpl-lite', 'ls-doing',    'ls-review',  '提审')   ON CONFLICT (id) DO NOTHING"),
        ('INSERT INTO workflow_transitions (id, template_id, from_state_id, to_state_id, name) VALUES '
         "('lt-rev-plan',  'tmpl-lite', 'ls-review',   'ls-planning', '驳回')   ON CONFLICT (id) DO NOTHING"),
        ('INSERT INTO workflow_transitions (id, template_id, from_state_id, to_state_id, name) VALUES '
         "('lt-rev-done',  'tmpl-lite', 'ls-review',   'ls-done',    '通过')   ON CONFLICT (id) DO NOTHING"),

        ("INSERT INTO workspaces (id, name, key, description, type, status, phase, visibility, department_id, owner_id) "
         "VALUES ('ws-demo-001', 'AI-PM 演示项目', 'DEMO', 'AI 项目管理平台演示空间，包含完整研发流程示例', "
         "'PROJECT', 'ACTIVE', 'PLANNING', 'PUBLIC', 'dept-001', 'admin-001') "
         "ON CONFLICT (id) DO NOTHING"),
    ]
